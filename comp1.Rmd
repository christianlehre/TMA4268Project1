...
```
We see that the argmax classifier yields a lower misclassification error on the test set, i.e the argmax classifier is preferrable.
...
&= \sigma^2(X^TX)^{-1}
\end{aligned}
>>>>>>> e3921ff946a17a1b95b31b6913d11e64b6efb41f
$$
...
$$
  \begin{aligned}
    ...
    &\\
    &= x_0^T \text{Var}[\boldsymbol{\boldsymbol{\hat{\beta}}}]x_0 \\
    &\\
...
sqbias=function(lambda,X,x0,beta)
{
  ...
  value=(t(x0)%*%(diag(p)+lambda*solve(t(X)%*%X))%*%beta-t(x0)%*%beta)^2
  return(value)
}
...
```
We observe that the 35. lambda value 
